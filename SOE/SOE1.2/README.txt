README.txt
============================================================
SOE1.2：未知入射能量 2-hit 多圆锥与球面 SOE 联合重建程序说明
============================================================
...
重要说明：
    ...
    SOE1.2 位于 D:\CodexForSR\SOE\SOE1.2。它保留 SOE1 的物理模型、
    事件筛选、随机链和图像分析，只替换批处理进程调度与内存保护。
...
如果服务器目录变化，只修改 root_directory；ROOT 不需要复制进 SOE1.2。
在 SourceRecon 仓库根目录运行，并把日志放在 SOE 文件夹：
    ...
    /home/ezqi/miniconda3/envs/py38/bin/python -u \
        SOE/SOE1.2/main_geant4.py \
        --config SOE/SOE1.2/config/geant4_batch.json \
        --workers 5 \
        2>&1 | tee SOE/jcding_batch_SOE1_2.log
...
因此横向和纵向基向量均写成负号。它只修正白色真值星号、角误差与 R68
等事后评估，不改变 SOE 重建结果。如果以后换用命名约定不同的模拟数据，
只修改这三个向量和角度。

每个文件有千万级 EventID。批量配置默认：

    按 100 MB 分块读取 ROOT，以降低每个worker的瞬时内存；
    扫描完整文件以保留真实总事件统计；
...
批处理默认：
    ...
    batch.parallel_workers = 5

这里使用多进程而不是 Python 线程。每个 ROOT 的 IO、能谱、kernel 和 SOE
链都完全独立，多进程可以真正并行使用多个 CPU 核。worker 数可按服务器
内存修改。SOE1.2 中这个数字是“最大并发上限”，不是强制始终同时运行
这么多进程。内存保护器可能自动少开或降低并发。

每个 worker 都会分块读取一个 ROOT，并各自保留最多 100000 个谱学事件和
2000 个成像事件，因此内存占用会近似随 parallel_workers 成比例增加。
不要同时启动两个批处理程序写同一个 output.directory。

命令行的 --workers 会临时覆盖 JSON，方便在不同服务器上调节；省略时就
使用 JSON 中的 parallel_workers。

SOE1.2 的 batch.memory_guard 默认参数：
...
启动新任务前，调度器会把“已有worker尚未增长到峰值的内存”和“新worker
预计峰值”一并计入。如果预计启动后无法保留 5 GiB 可用内存，就暂不启动。
运行中若可用内存低于保护线，程序会主动终止最新启动的一组、把并发上限
降低一档，并把该组重新排队。这样可在Linux OOM Killer介入前保护主程序和
其他任务。

每组数据由一次性worker处理。组完成后worker退出，确保NumPy、Pandas、
healpy和Matplotlib占用的内存真正归还操作系统。若worker被SIGKILL、段错误
或底层库异常直接终止，调度器只重试该组，不会像旧进程池一样让全部90组
一起失败。

内存过程持续追加到：

    output/jcding_batch_SOE1_2/memory_telemetry.csv

其中包含可用内存、Swap剩余、活动worker数、当前并发上限、活动worker
RSS总和以及降级/重试事件。batch_manifest.json 的 scheduler_statistics
保存本次运行最低可用内存、最大实际并发、内存保护次数和最终并发上限。

如果输出中已有旧版完整 numerical 文件，程序会检查真值方向和两张新图：

    02a_front_hemisphere_raw_pixel_map.png
    02b_front_hemisphere_smoothed_map.png

缺少时只重新生成分析文件和图片，不会重新运行昂贵的 ROOT 读取与 SOE。
...
02_front_hemisphere_flat.png 保留原始 HEALPix 中心散点，便于检查像素布局。

02a_front_hemisphere_raw_pixel_map.png 把每个 HEALPix pixel 的原始值完整
铺到单位圆盘，不做平滑，不制造额外角分辨率，是正式数值图的直观版本。

02b_front_hemisphere_smoothed_map.png 先在球面上做默认 5° FWHM 的高斯显示
平滑，再投影成连续热力图。该平滑只影响 PNG，不进入质量指标、事件后验或
SOE 采样。宽度可用 analysis.map_smoothing_fwhm_deg 修改。

09_dataset_dashboard.png 的左上角使用 smoothed map，overall/montages/ 也
使用 smoothed map。03_source_centered_zoom.png 以模拟真值为原点，横纵轴
直接表示角偏差 degree；白色星号是真值，黑色叉号是后验质心。
