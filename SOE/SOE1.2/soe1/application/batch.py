...
from .memory import MemoryGuardPolicy
...
class SimulationBatchApplication:
    ...
    def run(self) -> Dict[str, str]:
        ...
        print(
            "SOE1.2 batch will run "
            + str(len(pending_tasks))
            + " pending tasks with at most "
            + str(effective_workers)
            + " worker process(es).",
            flush=True,
        )

        memory_policy = MemoryGuardPolicy.from_batch_config(batch_config)
        scheduler_statistics = {
            "configured_max_workers": int(effective_workers),
            "initial_worker_limit": 0,
            "final_worker_limit": 0,
            "maximum_active_workers": 0,
            "completed_process_count": 0,
            "memory_pressure_event_count": 0,
            "abrupt_exit_event_count": 0,
            "retried_task_count": 0,
            "minimum_available_gib": None,
            "peak_active_worker_rss_gib": 0.0,
            "telemetry_csv": str(
                output_root / "memory_telemetry.csv"
            ),
        }
        if pending_tasks:
            scheduler = MemoryAwareBatchScheduler(
                worker_function=_run_dataset_task,
                maximum_workers=effective_workers,
                memory_policy=memory_policy,
                telemetry_path=output_root / "memory_telemetry.csv",
            )
            for result in scheduler.iter_results(pending_tasks):
            ...
            scheduler_statistics = scheduler.statistics.to_dict()
        ...
        if metrics:
            ...
            output_paths = self._write_failure_only_report(
                output_root,
                failures,
            )
            print(
                "No successful dataset is available; overall image "
                "report was skipped. Failure files and manifest were kept.",
                flush=True,
            )
    ...
    @staticmethod
    def _write_failure_only_report(
        output_root,
        failures,
    ) -> Dict[str, str]:
        overall = Path(output_root) / "overall"
        overall.mkdir(parents=True, exist_ok=True)
        json_path = overall / "failed_datasets.json"
        ...
        csv_path = overall / "failed_datasets.csv"
        field_names = sorted(
            {
                key
        ...
        return {
            "failed_datasets_csv": str(csv_path),
            "failed_datasets_json": str(json_path),
        }
        ...
        payload = {
            ...
            "memory_guard": {
                ...
                "minimum_available_gib": (
                    memory_policy.minimum_available_gib
                ),
                "estimated_worker_peak_gib": (
                    memory_policy.estimated_worker_peak_gib
                ),
                "poll_interval_seconds": (
                    memory_policy.poll_interval_seconds
                ),
                "maximum_abrupt_retries": (
                    memory_policy.maximum_abrupt_retries
                ),
            },
