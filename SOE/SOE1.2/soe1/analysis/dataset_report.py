...
class DatasetQualityReporter:
    ...
    def __init__(
        ...
        self.flat_map_grid_size = int(
            self.config.get("flat_map_grid_size", 700)
        )
        self.map_smoothing_fwhm_deg = float(
            self.config.get("map_smoothing_fwhm_deg", 5.0)
        )
        if self.flat_map_grid_size < 100:
            raise ValueError("analysis.flat_map_grid_size 不能小于 100。")
        if self.map_smoothing_fwhm_deg <= 0.0:
            raise ValueError(
                "analysis.map_smoothing_fwhm_deg 必须大于 0。"
            )
        self._front_projection_cache = {}
    ...
    def _write_figures(
        ...
        paths["raw_pixel_map_png"] = self._filled_front_hemisphere(
            sky=sky,
            expected=expected,
            metrics=metrics,
            smoothing_fwhm_deg=None,
            file_name="02a_front_hemisphere_raw_pixel_map.png",
            title="Raw HEALPix posterior on the detector-facing hemisphere",
        )
        paths["smoothed_map_png"] = self._filled_front_hemisphere(
            sky=sky,
            expected=expected,
            metrics=metrics,
            smoothing_fwhm_deg=self.map_smoothing_fwhm_deg,
            file_name="02b_front_hemisphere_smoothed_map.png",
            title=(
                "Smoothed detector-facing posterior "
                + f"(Gaussian FWHM {self.map_smoothing_fwhm_deg:g} deg)"
            ),
        )
    ...
    def _filled_front_hemisphere(
        ...
        figure, axis = plt.subplots(figsize=(7.4, 6.6))
        self._draw_filled_front_map(
            axis=axis,
            sky=sky,
            expected=expected,
            metrics=metrics,
            smoothing_fwhm_deg=smoothing_fwhm_deg,
            title=title,
        )
        figure.tight_layout()
        figure.savefig(path, dpi=self.dpi)
    ...
    def _draw_filled_front_map(
        ...
        projected = self._project_front_hemisphere(
            sky,
            smoothing_fwhm_deg,
        )
        image = axis.imshow(
    ...
    def _project_front_hemisphere(
        self,
        sky,
        smoothing_fwhm_deg,
    ) -> np.ma.MaskedArray:
        """
        把 HEALPix 后验精确铺到规则圆盘网格。

        raw 模式只做像素查询，不改变任何 HEALPix 数值；smooth 模式先在
        球面上做配置宽度的高斯平滑，再投影到相同圆盘。圆盘外保持透明。
        """

        cache_key = (
            None
            if smoothing_fwhm_deg is None
            else float(smoothing_fwhm_deg)
        )
        cached = self._front_projection_cache.get(cache_key)
        ...
        try:
            ...
            raise ImportError(
                "填充式半球图需要 healpy：pip install healpy"
            ) from exc

        ordered = sky.sort_values("pixel_index")
        pixel_indices = ordered["pixel_index"].to_numpy(dtype=int)
        expected_indices = np.arange(len(ordered), dtype=int)
        if not np.array_equal(pixel_indices, expected_indices):
            raise ValueError("sky_map.csv 的 HEALPix pixel_index 不连续。")

        map_values = ordered["posterior_mean_count"].to_numpy(dtype=float)
        nside = int(hp.npix2nside(len(map_values)))
        nested = self._healpix_nested()
        if smoothing_fwhm_deg is not None:
            if nested:
                map_values = hp.reorder(map_values, n2r=True)
                nested = False
            map_values = hp.smoothing(
                map_values,
                fwhm=np.deg2rad(float(smoothing_fwhm_deg)),
                verbose=False,
            )
        ...
        coordinates = np.linspace(
            -1.0,
            1.0,
            self.flat_map_grid_size,
        )
        x_grid, y_grid = np.meshgrid(coordinates, coordinates)
        radius_squared = x_grid * x_grid + y_grid * y_grid
        disk = radius_squared <= 1.0
        z_grid = np.zeros_like(x_grid)
        z_grid[disk] = -np.sqrt(
            np.clip(1.0 - radius_squared[disk], 0.0, 1.0)
        )

        projected = np.full(x_grid.shape, np.nan, dtype=float)
        pixels = hp.vec2pix(
            nside,
            x_grid[disk],
            y_grid[disk],
        ...
        projected[disk] = map_values[pixels]
        masked = np.ma.masked_invalid(projected)
        self._front_projection_cache[cache_key] = masked
        return masked

    def _healpix_nested(self) -> bool:
        path = self.numerical_directory / "configuration_snapshot.json"
        try:
            with path.open("r", encoding="utf-8") as file:
                configuration = json.load(file)
            return bool(
                configuration.get("reconstruction", {}).get(
                    "healpix_nested",
                    False,
                )
            )
    ...
    def _dashboard(
        ...
        self._draw_filled_front_map(
            ...
            title=(
                "Smoothed detector-facing posterior "
                + f"({self.map_smoothing_fwhm_deg:g} deg FWHM)"
            ),
        )
        self._draw_zoom_map(
