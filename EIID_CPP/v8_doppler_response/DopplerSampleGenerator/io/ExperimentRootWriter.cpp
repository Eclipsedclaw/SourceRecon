...
void ExperimentRootWriter::open()
{
    ...
    tree_->Branch(CalibrationSchema::eventId.data(), &buffer_.eventId);
    ...
    tree_->Branch(
        ...
        &buffer_.detectorKinematicsValid
    );
    ...
    tree_->Branch(
        ...
        &buffer_.truthThetaKinDegree
    );
    tree_->Branch(CalibrationSchema::truthArm.data(), &buffer_.truthArmDegree);
    tree_->Branch(
    ...
    tree_->Branch(
        "detector_theta_geo_degree",
        &buffer_.detectorThetaGeoDegree
    );
    tree_->Branch(
        CalibrationSchema::detectorScatterAngle.data(),
        &buffer_.detectorThetaKinDegree
    );
    tree_->Branch(
        CalibrationSchema::detectorArm.data(),
        &buffer_.detectorArmDegree
    );
    tree_->Branch(
...
void ExperimentRootWriter::finish(const ExperimentCounters& counters)
{
    ...
    TParameter<double> sourceEnergy{
        CalibrationSchema::sourceEnergyMetadata.data(),
        config_->sourceEnergyMeV()
    };
    ...
    TNamed schema{
        ...
        CalibrationSchema::description().data()
    };
    schema.Write();
    sourceEnergy.Write();
    sourceTheta.Write();
