...
```text
Reconstruction/{config,include,src,io,tests} -> EIID_Recon_V8
```

## Build

On a new machine, configure dependencies once from the V8 root:
...
This automatically builds the common and response-kernel libraries. The executable is `EIID_Recon_V8`.

## Run

```bash
./EIID_Recon_V8 config/recon_config.json
```
...
V8 adds `response_kernel.type` (`fixed_gaussian`, `voigt`, `double_gaussian`, or `histogram`), `fixed_sigma_degree`, `calibration_file`, `parameter_tree`, and `histogram_name`. Examples are `recon_config.json`, `recon_voigt.json`, and `recon_double_gaussian.json`. A V7 JSON without this object remains a fixed-Gaussian run.
