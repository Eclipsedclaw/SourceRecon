...
编译时会自动生成公共库和响应核库。最终程序名为 `EIID_Recon_V8`。
...
V8 新增的 `response_kernel` 对象：

| 参数 | 含义 |
|---|---|
| `type` | `fixed_gaussian`、`voigt`、`double_gaussian` 或 `histogram` |
| `fixed_sigma_degree` | 固定高斯宽度 |
| `calibration_file` | ResponseCalibration 输出的 ROOT 文件 |
| `parameter_tree` | Voigt/双高斯参数 Tree |
| `histogram_name` | 经验三维 PDF 名 |

示例为 `recon_config.json`、`recon_voigt.json` 和 `recon_double_gaussian.json`。旧 V7 JSON 不含该对象时仍自动使用固定高斯。
