...
This module composes compact ROOT events, a HEALPix grid, an absolute-efficiency map, the EIID response kernel, and an LM-MLEM solver. It builds independently as `EIID_Recon_V5`.
...
`EiidResponse` supplies the conditional event term `q_ij`. V5 constructs the complete response and forward prediction as
...
`RootEfficiencyReader` rejects files with mismatched Nside, ordering, direction count, energy count/range, missing source radius, or a non-V5 efficiency definition. `cell_index` duplicates, omissions, invalid values, and out-of-range indices are rejected by `AbsoluteEfficiencyMap`.
