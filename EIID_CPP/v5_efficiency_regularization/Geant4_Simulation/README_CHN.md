...
- `AbsoluteEfficiencyEstimator`：用 Jeffreys 平滑估计锥内触发概率，再归一化到 (4\pi) 各向同性发射。
...
对于实际参与模拟的 cell，即使 `valid = 0`，结果也只是很小的正效率，不会被误判为绝对不可能。`cone-emitted = 0` 表示 cell 位于配置的模拟域之外，仍写为严格的零。平滑项在高统计量下自动消失。

每个单元只保存 `cell_index` 与 `efficiency`，网格和科学定义等元数据只在 ROOT 文件层级保存一次。
