...
V5 provides five user-facing programs:
...
| `Reconstruction/EIID_Recon_V5` | Run EIID + LM-MLEM reconstruction |
| `Visualization/eiid_plotter` | Generate sky map, spectrum, and containment PNGs |
...
The project does not require a permanently activated Conda shell. Configure the machine once from the V5 root:
...
Subsequent terminals only need `make v5-all`; environment activation is unnecessary. After moving to another server, delete or overwrite `config/local.mk` by running `make configure` again.
...
```bash
cd Reconstruction
./EIID_Recon_V5 config/recon_config.json
```
...
PNG files are placed in `Visualization/figures/` by the default configuration.
...
Compact `events.root` data are independent of the efficiency estimator. Older compact events remain valid in V5 when their Tree/Branches, units, and `r1=ch2, r2=ch1` convention match. The artifact that must be regenerated by V5 is the absolute-efficiency map.
...
```bash
./Reconstruction/EIID_Recon_V5 Reconstruction/config/recon_config.json
