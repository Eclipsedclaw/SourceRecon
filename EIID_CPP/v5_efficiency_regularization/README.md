# EIID V5 Absolute-Efficiency Architecture

## 1. Purpose

V5 is a modular C++20 pipeline for Compton-camera direction–energy reconstruction. It keeps simulation, raw-data translation, grid resampling, reconstruction, and visualization independently compilable. Relative to V4, finite-Monte-Carlo zero counts in active cells are Jeffreys-regularized, and the LM-MLEM forward prediction now uses the complete `absolute efficiency × conditional event response` kernel. The V4 directory remains unchanged for audit and comparison.
...
`Reconstruction/` builds `EIID_Recon_V5`.
...
The inherited V3 files are preserved under `legacy_v3_snapshot/` for audit only and are not part of any V5 build target.
