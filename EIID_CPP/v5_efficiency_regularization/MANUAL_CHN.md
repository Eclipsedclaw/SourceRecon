...
| `Reconstruction/EIID_Recon_V5` | 执行 EIID + LM-MLEM 重建 |
...
项目不再要求每次激活 Conda 环境。在 V5 根目录对每台机器执行一次：
...
以后新开的终端直接运行 `make v5-all`，无需再激活环境。若把项目迁移到另一台服务器，再执行一次 `make configure` 覆盖旧的 `config/local.mk`。
...
精简 `events.root` 与效率估计算法彼此独立。只要事件 Tree/Branch、坐标单位和 `r1=ch2、r2=ch1` 约定一致，V4 以前生成的精简事件可以直接交给 V5；必须由 V5 重新生成的是绝对效率图。
