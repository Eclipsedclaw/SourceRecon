...
DetectorConstruction::DetectorConstruction(const ConfigManager& config)
  : fConfig(&config)
{
  ...
  fGap1 = 25 * mm;
  fGap2 = 35 * mm;
  fGeometryInfo = std::make_unique<DetectorGeometryInfo>(fGap1, fGap2);
}
...
{
  G4Material* worldMaterial = G4Material::GetMaterial(fConfig->worldMaterial());
  ...
  const G4double sourceRadius = fConfig->hemisphereRadiusMm() * mm;
  const G4double worldMargin = fConfig->worldMarginMm() * mm;
  const G4ThreeVector ch2Center = fGeometryInfo->ch2Center();
  const G4double worldHalfX = std::max(
      fGeometryInfo->detectorHalfXY(),
      std::abs(ch2Center.x()) + sourceRadius
  ) + worldMargin;
  const G4double worldHalfY = worldHalfX;
  const G4double worldHalfZ = std::max({
      std::abs(ch2Center.z() - sourceRadius),
      std::abs(ch2Center.z() + sourceRadius),
      std::abs(fGeometryInfo->detectorMinimumZ()),
      std::abs(fGeometryInfo->detectorMaximumZ())
  }) + worldMargin;
  G4GeometryManager::GetInstance()->SetWorldMaximumExtent(
      2.0 * std::max({worldHalfX, worldHalfY, worldHalfZ})
  );

  auto worldS = new G4Box("world", worldHalfX, worldHalfY, worldHalfZ);
...
void DetectorConstruction::ConstructSDandField()
{
  ...
  // 实验还必须知道主光子是否在两层之间的支撑材料发生过相互作用。
  // 这些 hit 的 chamber ID 保持 -1，不会混入晶体层的能量求和；它们只
  // 用来建立完整的主光子离散过程顺序。
  SetSensitiveDetector("YSO_Array_LV", aTrackerSD, true);
  SetSensitiveDetector("Si_LV", aTrackerSD, true);
  SetSensitiveDetector("FR4_LV", aTrackerSD, true);
...
{
  fGap1 = val;
  *fGeometryInfo = DetectorGeometryInfo(fGap1, fGap2);
...
const DetectorGeometryInfo& DetectorConstruction::GeometryInfo() const
{
