...
| `Reconstruction/EIID_Recon_V7` | 执行 EIID + LM-MLEM 重建 |
| `Visualization/eiid_plotter` | 生成基础物理图、模型无关质量图和 JSON 摘要 |
...
项目不再要求每次激活 Conda 环境。在 V7 根目录对每台机器执行一次：
...
以后新开的终端直接运行 `make v7-all`，无需再激活环境。若把项目迁移到另一台服务器，再执行一次 `make configure` 覆盖旧的 `config/local.mk`。
...
| `draw_energy_metrics` | 是否生成直接 FWHM 与最短强度区间图 |
| `draw_direction_metrics` | 是否生成球面切平面方向质心与协方差图 |
| `draw_energy_angle_map` | 是否生成能量—角距离联合图 |
| `write_quality_summary` | 是否输出 `quality_summary.json` |
...
| 参数 | 含义 |
|---|---|
| `energy_interval_fraction` | 最短连续能量区间包含的强度比例 |
| `gaussian_fit_enabled` | 是否额外尝试 Gaussian 诊断拟合 |
| `gaussian_fit_half_width_MeV` | 可选能量 Gaussian 的窗口半宽 |
| `direction_local_radius_degree` | 围绕数据方向峰的局部质心/协方差半径 |
| `direction_spectrum_radius_degree` | 最终能谱相对拟合方向的圆形门半径 |
...
默认目录为 `Visualization/figures/`，新增 `energy_resolution.png`、`direction_resolution.png`、`energy_angle_map.png` 和 `quality_summary.json`。真值只计算偏差和画参考标记，不参与找峰。Gaussian 不收敛时只标为 unavailable；直接 FWHM、最短强度区间、质心/协方差及 R50/R68/R90 仍然有效。程序使用 ROOT batch 模式，不需要 X11。
...
精简 `events.root` 与效率估计算法彼此独立。只要事件 Tree/Branch、坐标单位和 `r1=ch2、r2=ch1` 约定一致，V4 以前生成的精简事件可以直接交给 V7；必须由 V7 重新生成的是绝对效率图。
