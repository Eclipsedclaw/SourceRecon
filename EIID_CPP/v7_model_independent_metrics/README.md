# EIID V7 Model-Independent Metrics Architecture

## 1. Purpose

V7 is a modular C++20 pipeline for Compton-camera direction–energy reconstruction. It keeps simulation, raw-data translation, grid resampling, reconstruction, and visualization independently compilable. It inherits V5's Jeffreys-regularized absolute efficiency and complete LM-MLEM kernel unchanged. This version adds direct energy FWHM, shortest intensity intervals, tangent-plane direction centroid/covariance, an energy–angle view, and a machine-readable quality summary. Gaussian fitting is optional diagnostics only. V5 and V6 remain unchanged for comparison.
...
`Reconstruction/` builds `EIID_Recon_V7`.
...
- `LmMlemSolver`: current list-mode MLEM implementation. Its forward prediction includes absolute efficiency, while true out-of-domain zero-efficiency cells remain masked.
...
- `AbsoluteEfficiencyEstimator`: converts cone counts into the isotropic 4π probability. Every simulated cell uses the Jeffreys posterior mean:
  ...
  `N_cone = 0` identifies a cell outside the simulation domain and remains exactly zero. As `N_cone` grows, the estimator converges automatically to the original `N_valid/N_cone` estimator.
...
- `QualityAnalyzer`: shared model-independent analysis whose peak search does not use truth.
- `EnergyMetricsPlotter`: direct FWHM, shortest intensity interval, and cumulative spectrum.
- `DirectionMetricsPlotter`: weighted tangent-plane centroid and 1/2 RMS covariance ellipses.
- `EnergyAnglePlotter`: joint intensity versus energy and angular distance from truth.
- `QualitySummaryWriter`: writes `quality_summary.json` for batch comparisons.
...
Direction is analyzed on a local spherical tangent plane rather than as separate theta/phi distributions. Because MLEM weights are correlated, widths are descriptive quality metrics rather than formal confidence intervals; widths below the HEALPix pixel scale are flagged as under-resolved. Optional Gaussian failure does not invalidate primary metrics.

## 9. ROOT schemas

### Absolute efficiency file

Tree name is configurable; the default is `AbsoluteEfficiency`.
...
The inherited V3 files are preserved under `legacy_v3_snapshot/` for audit only and are not part of any V7 build target.
