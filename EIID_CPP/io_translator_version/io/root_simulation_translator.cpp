// D:\CodexForSR\EIID_CPP\io_translator_version\io\root_simulation_translator.cpp
...
namespace
{
// 当前 Geant4 Tree1 的固定原始 schema。
// 位置使用 x_post/y_post/z_post，与项目已有 legacy 物理基线保持一致。
constexpr const char* RAW_EVENT_ID_BRANCH = "eventID";
constexpr const char* RAW_CHAMBER_ID_BRANCH = "chamberID";
constexpr const char* RAW_X_BRANCH = "x_post";
constexpr const char* RAW_Y_BRANCH = "y_post";
constexpr const char* RAW_Z_BRANCH = "z_post";
constexpr const char* RAW_ENERGY_BRANCH = "eDep_MeV";

// 官方映射：0 是最前方 ch2，1 是后方 ch1。
constexpr Int_t CH2_CHAMBER_ID = 0;
constexpr Int_t CH1_CHAMBER_ID = 1;
...
// ch0 是否存在不会影响当前触发判定。
bool passesTrigger(const DigitizedEvent& event)
{
    const auto ch2 = event.find(CH2_CHAMBER_ID);
    const auto ch1 = event.find(CH1_CHAMBER_ID);

    return ch2 != event.end() && ch1 != event.end() && ch2->second.energyMeV > 0.0 && ch1->second.energyMeV > 0.0;
}

bool transformAndWriteEvent(Int_t eventId, const std::vector<RawStep>& steps, TranslatedRow& row, TTree& outputTree)
{
    const DigitizedEvent event = digitize(steps);

    if (!passesTrigger(event))
    {
        return false;
    }

    // 经过 TriggerPolicy 后，这两个 key 一定存在；at() 同时保留运行时边界检查。
    const DigitizedHit& ch2 = event.at(CH2_CHAMBER_ID);
    const DigitizedHit& ch1 = event.at(CH1_CHAMBER_ID);
...
void translateRawData(const IoConfig& config)
{
    ...
    requireBranch(rawTree, RAW_EVENT_ID_BRANCH);
    requireBranch(rawTree, RAW_CHAMBER_ID_BRANCH);
    requireBranch(rawTree, RAW_X_BRANCH);
    requireBranch(rawTree, RAW_Y_BRANCH);
    requireBranch(rawTree, RAW_Z_BRANCH);
    requireBranch(rawTree, RAW_ENERGY_BRANCH);

    TTreeReader reader{rawTree};
    TTreeReaderValue<Int_t> eventId{reader, RAW_EVENT_ID_BRANCH};
    TTreeReaderValue<Int_t> chamberId{reader, RAW_CHAMBER_ID_BRANCH};
    TTreeReaderValue<Double_t> x{reader, RAW_X_BRANCH};
    TTreeReaderValue<Double_t> y{reader, RAW_Y_BRANCH};
    TTreeReaderValue<Double_t> z{reader, RAW_Z_BRANCH};
    TTreeReaderValue<Double_t> energyMeV{reader, RAW_ENERGY_BRANCH};
    ...
    while (reader.Next())
    {
        ...
        else if (*eventId != bufferedEventId)
        {
            ...
            if (transformAndWriteEvent(bufferedEventId, eventBuffer, row, outputTree))
            {
                ++acceptedEventCount;
            }
