...
该模块把紧凑 ROOT 事件、HEALPix 网格、绝对探测效率、EIID 响应函数和 LM-MLEM 求解器组装起来，可独立编译为 `EIID_Recon_V6`。
...
`EiidResponse` 给出条件事件响应 `q_ij`。V6 先构造完整系统响应
...
若 Nside、排序、方向数、能量数或范围不匹配，或者缺少源半径、效率定义不属于 V6，`RootEfficiencyReader` 会直接报错。重复索引、缺失索引、越界索引和非法效率值由 `AbsoluteEfficiencyMap` 拒绝。
