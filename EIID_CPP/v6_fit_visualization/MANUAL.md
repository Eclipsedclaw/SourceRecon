...
V6 provides five user-facing programs:
...
| `Reconstruction/EIID_Recon_V6` | Run EIID + LM-MLEM reconstruction |
...
The project does not require a permanently activated Conda shell. Configure the machine once from the V6 root:
...
Subsequent terminals only need `make v6-all`; environment activation is unnecessary. After moving to another server, delete or overwrite `config/local.mk` by running `make configure` again.
...
| `draw_energy_fit` | Enable direction-gated energy fit and residual plot |
| `draw_direction_fit` | Enable spherical tangent-plane direction fit |
...
| Parameter | Meaning |
|---|---|
| `energy_fit_half_width_MeV` | Fit half-window around the data energy peak |
| `direction_energy_gate_sigma` | Direction-fit energy half-gate in preliminary-fit sigma |
| `direction_fit_radius_degree` | Local 2D fit radius around the data direction peak |
| `direction_spectrum_radius_degree` | Circular direction gate for the final energy fit |
...
The default directory is `Visualization/figures/`. V6 adds `energy_fit.png`, `direction_fit.png`, `energy_angle_map.png`, and `fit_summary.json`. Truth is used only for biases and markers, never peak selection. Fit widths are descriptive MLEM-image metrics and should be reported with R50/R68/R90.
...
Compact `events.root` data are independent of the efficiency estimator. Older compact events remain valid in V6 when their Tree/Branches, units, and `r1=ch2, r2=ch1` convention match. The artifact that must be regenerated by V6 is the absolute-efficiency map.
...
```bash
./Reconstruction/EIID_Recon_V6 Reconstruction/config/recon_config.json
