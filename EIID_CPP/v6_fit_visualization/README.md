# EIID V6 Fit-Visualization Architecture

## 1. Purpose

V6 is a modular C++20 pipeline for Compton-camera direction–energy reconstruction. It keeps simulation, raw-data translation, grid resampling, reconstruction, and visualization independently compilable. It inherits V5's Jeffreys-regularized absolute efficiency and complete LM-MLEM kernel unchanged, while adding local energy fitting, spherical tangent-plane direction fitting, an energy–angle correlation view, and a machine-readable fit summary. The V5 directory remains unchanged for audit and comparison.
...
`Reconstruction/` builds `EIID_Recon_V6`.
...
- `FitAnalyzer`: shared analysis service whose peak search does not use truth.
- `EnergyFitPlotter`: direction-gated local Gaussian fit and residuals.
- `DirectionFitPlotter`: energy-gated tangent-plane elliptical Gaussian and 1σ/2σ ellipses.
- `EnergyAnglePlotter`: joint intensity versus energy and angular distance from truth.
- `FitSummaryWriter`: writes `fit_summary.json` for batch comparisons.
...
Direction is fitted on a local spherical tangent plane rather than as separate theta/phi distributions. Because MLEM weights are correlated, Gaussian widths are descriptive quality metrics rather than formal confidence intervals; widths below the HEALPix pixel scale are flagged as under-resolved.
...
The inherited V3 files are preserved under `legacy_v3_snapshot/` for audit only and are not part of any V6 build target.
