...
| `Reconstruction/EIID_Recon_V6` | 执行 EIID + LM-MLEM 重建 |
| `Visualization/eiid_plotter` | 生成基础物理图、拟合诊断图和 JSON 摘要 |
...
项目不再要求每次激活 Conda 环境。在 V6 根目录对每台机器执行一次：
...
以后新开的终端直接运行 `make v6-all`，无需再激活环境。若把项目迁移到另一台服务器，再执行一次 `make configure` 覆盖旧的 `config/local.mk`。
...
| `draw_energy_fit` | 是否生成方向门控能量拟合与残差图 |
| `draw_direction_fit` | 是否生成球面切平面二维方向拟合图 |
| `draw_energy_angle_map` | 是否生成能量—角距离联合图 |
| `write_fit_summary` | 是否输出 `fit_summary.json` |
...
| 参数 | 含义 |
|---|---|
| `energy_fit_half_width_MeV` | 围绕数据能谱最高峰的拟合半宽 |
| `direction_energy_gate_sigma` | 方向拟合能量门的半宽，单位为初步拟合 σ |
| `direction_fit_radius_degree` | 围绕数据方向峰的局部二维拟合半径 |
...
默认目录为 `Visualization/figures/`，新增 `energy_fit.png`、`direction_fit.png`、`energy_angle_map.png` 和 `fit_summary.json`。真值只计算偏差和画参考标记，不参与找峰。拟合宽度是 MLEM 图像的描述量，应与 R50/R68/R90 一起报告。程序使用 ROOT batch 模式，不需要 X11。
...
精简 `events.root` 与效率估计算法彼此独立。只要事件 Tree/Branch、坐标单位和 `r1=ch2、r2=ch1` 约定一致，V4 以前生成的精简事件可以直接交给 V6；必须由 V6 重新生成的是绝对效率图。
