...
void EventAction::EndOfEventAction(const G4Event* event)
{
    ...
    const G4HCofThisEvent* collections = event->GetHCofThisEvent();
    ...
    if (hitsCollectionId_ < 0)
    {
        hitsCollectionId_ = G4SDManager::GetSDMpointer()->GetCollectionID(
            "/TrackerChamberSD/TrackerHitsCollection"
        );

        if (hitsCollectionId_ < 0)
        {
            throw std::runtime_error{
                "Cannot find /TrackerChamberSD/TrackerHitsCollection."
            };
        }
    }
    ...
    const bool validTrigger =
        frontLayer.energy > threshold && rearLayer.energy > threshold;
    runAction_->recordEvent(cell.flatIndex, validTrigger);
