...
G4bool TrackerSD::ProcessHits(G4Step* aStep, G4TouchableHistory*)
{
  ...
  // 连续电离产生的能量沉积有时出现在由几何边界（Transportation）终止的 step。
  // 如果无条件丢掉这类 step，两个晶体层的总能量会被低估，进而伪造能量拖尾。
  // 因此只有“Transportation 且确实没有能量沉积”时才忽略该 step。
  // 主光子的零沉积 Transportation step 仍要保留：它能证明光子是否真的
  // 进入了 ch2/ch1。其他粒子的零沉积 Transportation step 没有分析价值。
  const G4bool isPrimaryGamma =
      track->GetCreatorProcess() == nullptr && particleName == "gamma";
  if (processname == "Transportation" && edep <= 0.0 && !isPrimaryGamma)
  {
      return false;
  }
  ...
  newHit->SetMomentum(preStepPoint->GetMomentum());
  newHit->SetPostMomentum(postStepPoint->GetMomentum());
  newHit->SetAngle(theta);
  newHit->SetEdep(edep);
  newHit->SetKineticEnergy(postStepPoint->GetKineticEnergy());
  newHit->SetPreKineticEnergy(preStepPoint->GetKineticEnergy());
...
void TrackerSD::EndOfEvent(G4HCofThisEvent*)
{
  ...
  // V7 由 EventAction 直接消费 hits，不再输出千万行原始 Step ntuple。
