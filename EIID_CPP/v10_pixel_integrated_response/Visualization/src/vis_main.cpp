#include "containment_plotter.h"
#include "direction_metrics_plotter.h"
#include "energy_angle_plotter.h"
#include "energy_metrics_plotter.h"
#include "iplotter.h"
#include "quality_analysis.h"
#include "quality_summary_writer.h"
#include "reconstruction_data.h"
#include "skymap_plotter.h"
#include "spectrum_plotter.h"
#include "vis_config.h"

#include <TROOT.h>
#include <TStyle.h>

#include <exception>
#include <filesystem>
#include <iostream>
#include <memory>
#include <string>
#include <vector>

int main(int argc, char* argv[])
{
    try
    {
        if (argc > 2)
        {
            std::cerr << "Usage: eiid_plotter [path/to/plot_config.json]\n";
            return 1;
        }

        const std::filesystem::path configPath = argc == 2
            ? std::filesystem::path{argv[1]}
            : std::filesystem::path{"config/plot_config.json"};

        const VisConfig config{configPath};

        std::filesystem::create_directories(config.outputDirectory());

        // 绘图程序不需要打开 X11 窗口，直接在批处理模式下生成 PNG。
        gROOT->SetBatch(kTRUE);
        gStyle->SetOptStat(0);
        gStyle->SetTitleFontSize(0.035);

        // result.root 只读取一次，之后所有 Plotter 共享同一份只读数据。
        const ReconstructionData data = ReconstructionDataReader::read(config);

        // 三张质量诊断图和 JSON 摘要依赖同一组分析结果。
        // 这里统一计算一次，再用 shared_ptr 只读共享，避免重复统计。
        const bool needsQualityAnalysis =
            config.drawEnergyMetrics() ||
            config.drawDirectionMetrics() ||
            config.drawEnergyAngleMap() ||
            config.writeQualitySummary();
        std::shared_ptr<const QualityAnalysisResult> qualityAnalysis;

        if (needsQualityAnalysis)
        {
            qualityAnalysis = std::make_shared<const QualityAnalysisResult>(
                QualityAnalyzer::analyze(data, config)
            );
        }

        std::vector<std::unique_ptr<IPlotter>> plotters;

        if (config.drawSkymap())
        {
            plotters.push_back(std::make_unique<SkymapPlotter>());
        }

        if (config.drawSpectrum())
        {
            plotters.push_back(std::make_unique<SpectrumPlotter>());
        }

        if (config.drawContainment())
        {
            plotters.push_back(std::make_unique<ContainmentPlotter>());
        }

        if (config.drawEnergyMetrics())
        {
            plotters.push_back(
                std::make_unique<EnergyMetricsPlotter>(qualityAnalysis)
            );
        }

        if (config.drawDirectionMetrics())
        {
            plotters.push_back(
                std::make_unique<DirectionMetricsPlotter>(qualityAnalysis)
            );
        }

        if (config.drawEnergyAngleMap())
        {
            plotters.push_back(
                std::make_unique<EnergyAnglePlotter>(qualityAnalysis)
            );
        }

        if (plotters.empty() && !config.writeQualitySummary())
        {
            std::cout << "All visualization switches are disabled.\n";
            return 0;
        }

        for (const std::unique_ptr<IPlotter>& plotter : plotters)
        {
            plotter->plot(data, config);
            std::cout << "Generated " << plotter->name()
                      << " in " << config.outputDirectory().string() << '\n';
        }

        if (config.writeQualitySummary())
        {
            QualitySummaryWriter::write(*qualityAnalysis, config);
            std::cout << "Generated quality summary in "
                      << config.outputDirectory().string() << '\n';
        }
    }
    catch (const std::exception& error)
    {
        std::cerr << "Visualization error: " << error.what() << '\n';
        return 1;
    }

    return 0;
}
