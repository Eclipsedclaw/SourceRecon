...
Run `make configure` once from the V10 root on a new machine. Afterwards:
...
The default output is `translated_events.root`, so the external workflow input
`events.root` is not overwritten. After validation, explicitly copy it or point
Reconstruction at it. Skip this module when Geant4 Mode 1 already produced a
suitable compact event file.
