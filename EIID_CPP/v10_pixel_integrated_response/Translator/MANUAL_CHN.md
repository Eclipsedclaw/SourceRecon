...
新机器先在 V10 根目录执行一次 `make configure`，以后直接运行：
...
默认输出为根目录 `translated_events.root`，不会覆盖完整流程读取的外部 `events.root`。检查输出后，如需将其作为重建输入，应由使用者明确复制或修改 Reconstruction 配置。若 Geant4 Mode 1 已直接生成合格精简事件，则无需运行 Translator。
