...
)
{
    ...
    TTree tree{config.outputResultTree().c_str(), "EIID V10 direction-energy image"};
    ULong64_t cellIndex{};
    ULong64_t pixelId{};
    Double_t weight{};
    Double_t theta{};
    ...
    TNamed{
        ...
        config.pixelIntegration().strategy.c_str()
    }.Write();
    TParameter<int>{
        "integration_nside",
        config.pixelIntegration().integrationNside
    }.Write();
    const Long64_t refinement = static_cast<Long64_t>(
        config.pixelIntegration().integrationNside / grid.healpixNside()
    );
    TParameter<Long64_t>{
        "samples_per_pixel",
        refinement * refinement
    }.Write();
    TParameter<int>{"iteration_count", config.iterationCount()}.Write();
    TNamed{
        "response_kernel_type",
        config.responseKernel().type.c_str()
    }.Write();
    TParameter<Long64_t>{
        "direction_count",
