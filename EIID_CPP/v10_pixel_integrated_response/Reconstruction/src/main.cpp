...
int main(int argc, char* argv[])
{
    try
    {
            ...
            (validateEventsOnly && argc != 3))
        {
            std::cerr
                << "Usage:\n"
                << "  EIID_Recon_V10 [path/to/recon_config.json]\n"
                << "  EIID_Recon_V10 --validate-events-only path/to/recon_config.json\n";
            return 1;
        }
        ...
        const std::unique_ptr<ISystemResponseEvaluator> responseEvaluator =
            SystemResponseFactory::create(config, grid, *responseKernel);
        ...
        std::cout << "EIID V10 reconstruction completed.\n"
                  << "  events: " << events.size() << '\n'
                  ...
                  << "  samples per pixel: "
                  << responseEvaluator->samplesPerPixel() << '\n'
                  << "  result: " << config.outputResultFile().string() << '\n';
        return 0;
    }
    catch (const std::exception& error)
    {
        std::cerr << "EIID V10 error: " << error.what() << '\n';
        return 1;
    }
}
