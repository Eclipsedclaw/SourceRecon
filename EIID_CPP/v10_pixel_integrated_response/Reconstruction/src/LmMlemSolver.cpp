...
LmMlemSolver::LmMlemSolver(
    ...
    const ISystemResponseEvaluator& responseEvaluator
)
    : grid_{&grid},
      efficiency_{&efficiency},
      config_{&config},
      responseEvaluator_{&responseEvaluator}
{
         ...
         ++iteration)
    {
        ...
        for (const Event& event : events)
        {
            ...
            for (std::size_t cell = 0; cell < cells.size(); ++cell)
            {
                ...
                // The injected evaluator returns the conditional event-shape
                // term q_ij (point value or subpixel average). The complete
                // system response is
                ...
                const Decimal conditionalResponse =
                    responseEvaluator_->evaluate(event, cells[cell]);
                response[cell] =
