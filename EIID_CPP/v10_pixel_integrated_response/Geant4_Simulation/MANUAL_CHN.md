...
新机器先在 V10 根目录运行一次 `make configure`。如果脚本没有自动找到 Geant4，可明确指定：
...
把 `mode` 设为 `0`，默认生成 `runs/latest/efficiency/absolute_efficiency_master.root`。实际参与模拟但零触发的 cell 使用 `(k+0.5)/(N+1)` 平滑；未参与模拟的 cell 仍为严格零。正式标定必须逐步增加 `particles_per_cell`，并分别增加粒子数和发射锥安全余量检查结果是否收敛。粒子数增大时，平滑结果自动趋近普通的 `k/N`。
