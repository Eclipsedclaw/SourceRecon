...
```text
Geant4_Simulation/{config,include,src,io,tests} -> geant4_simulator
```

## 编译与运行

本次全零修复优先在本实验根目录运行 `make smoke`，通过后运行
`make smoke-plot`。配置为本模块的 `config/smoke_config.json`，输出在
实验根目录的 `runs/smoke_test/`，不会替换完整效率图。详细验收标准见
实验根目录的 `MANUAL_CHN.md`。

新机器先在本实验根目录运行一次 `make configure`。如果脚本没有自动找到 Geant4，可明确指定：
...
把 `mode` 设为 `0`，默认生成 `runs/raw_efficiency/raw_efficiency_master.root`。主 `efficiency` 分支严格使用 `(k/N) * Omega_cone/(4*pi)`；零触发 cell 保持严格零。`regularized_efficiency` 只用于对照旧公式。正式判断前必须增加 `particles_per_cell` 并检查 raw 结果是否收敛。
