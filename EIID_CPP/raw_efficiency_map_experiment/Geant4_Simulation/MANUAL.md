...
On a new machine, run `make configure` once from the V10 root. If Geant4 cannot be discovered automatically, use:
...
Use Mode 0 to create `runs/raw_efficiency/raw_efficiency_master.root`. The main `efficiency` branch is the unsmoothed `(k/N) * Omega_cone/(4*pi)` estimate, so zero-trigger cells remain exactly zero. The regularized branch is stored only for comparison. Increase `particles_per_cell` to test convergence.
