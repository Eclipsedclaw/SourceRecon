...
) const
{
    ...
    TTree tree{
        ...
        "Direction-energy raw absolute detection efficiency with diagnostics"
    };
    ULong64_t cellIndex{};
    Double_t efficiency{};
    Double_t regularizedEfficiency{};
    ULong64_t emittedCount{};
    ULong64_t validCount{};
    Double_t coneSolidAngleFraction{};
    tree.Branch(EfficiencyFileSchema::cellIndexBranch, &cellIndex);
    tree.Branch(EfficiencyFileSchema::efficiencyBranch, &efficiency);
    tree.Branch(
        EfficiencyFileSchema::regularizedEfficiencyBranch,
        &regularizedEfficiency
    );
    tree.Branch(EfficiencyFileSchema::emittedCountBranch, &emittedCount);
    tree.Branch(EfficiencyFileSchema::validCountBranch, &validCount);
    tree.Branch(
        EfficiencyFileSchema::coneSolidAngleFractionBranch,
    ...
    for (std::size_t index = 0; index < grid_->fullCellCount(); ++index)
    {
        ...
        coneSolidAngleFraction =
            AbsoluteEfficiencyEstimator::solidAngleFraction(cone.halfAngle);
        efficiency = AbsoluteEfficiencyEstimator::estimateRaw(
            validCounts[index],
            emittedCounts[index],
            cone.halfAngle
        );
        regularizedEfficiency = AbsoluteEfficiencyEstimator::estimate(
            validCounts[index],
            emittedCounts[index],
            cone.halfAngle
        );
    ...
    // 保存预期事例数，便于小测试独立核对是否完整计数。
    TParameter<Long64_t>{"requested_event_count", static_cast<Long64_t>(grid_->totalEventCount())}.Write();
    TParameter<double>{
