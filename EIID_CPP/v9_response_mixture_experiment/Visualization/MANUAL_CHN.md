...
make visualization
make run-visualization          # 只画标定状态为 usable 的模型
```
...
V9 提供 `plot_voigt.json`、`plot_double_gaussian.json` 和 `plot_gaussian_lorentzian_mixture.json`。直接运行时由使用者负责确认相应标定已收敛；推荐使用根目录 `make visualize-eligible` 自动门控。
