...
├── src/      ROOT 绘图、质量分析和入口
├── config/   含三个模型各自的 plot_*.json
├── Makefile
├── README.md / README_CHN.md
└── MANUAL.md / MANUAL_CHN.md
```

Visualization 是独立的 ROOT 程序，只读取结果 ROOT 和真值 JSON。V9 保留“模型无关指标主导”设计，并用三份独立配置查看 Voigt、双高斯和高斯+洛伦兹混合结果。默认输出集中到 `runs/latest/visualization/<model>/`。
