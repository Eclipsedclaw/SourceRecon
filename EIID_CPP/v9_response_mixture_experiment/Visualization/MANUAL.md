...
```bash
cd v9_response_mixture_experiment
make configure
make visualization
...
V9 provides `plot_voigt.json`, `plot_double_gaussian.json`, and `plot_gaussian_lorentzian_mixture.json`. Prefer the root `make visualize-eligible` target so failed calibration models are skipped automatically.
