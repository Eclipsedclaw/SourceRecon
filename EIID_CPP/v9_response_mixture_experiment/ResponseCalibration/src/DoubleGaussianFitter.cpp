...
ResponseFitResult DoubleGaussianFitter::fit(TH1D& histogram) const
{
    ...
    result.model = "double_gaussian";
    result.meanDegree = function.GetParameter(1);
    result.coreSigmaDegree = coreSigma;
    result.tailSigmaDegree = tailSigma;
    result.coreWeight = coreWeight;
    ...
    result.converged = status.Get() != nullptr &&
        ...
        std::isfinite(result.coreSigmaDegree) &&
        std::isfinite(result.tailSigmaDegree) &&
        std::isfinite(result.coreWeight);
