...
On a new machine, configure dependencies once from the V9 root:
...
This automatically builds the common and response-kernel libraries. The executable is `EIID_Recon_V9`.
...
V9 supports `response_kernel.type` values `fixed_gaussian`, `voigt`, `double_gaussian`, `gaussian_lorentzian_mixture`, and `histogram`. Examples are `recon_config.json`, `recon_voigt.json`, `recon_double_gaussian.json`, and `recon_gaussian_lorentzian_mixture.json`.
...
Use root target `make reconstruct-eligible` to run only models marked usable by calibration. Outputs default to `runs/latest/reconstruction/result_<model>.root`.

External-event preflight only:

```bash
./Reconstruction/EIID_Recon_V9 --validate-events-only \
