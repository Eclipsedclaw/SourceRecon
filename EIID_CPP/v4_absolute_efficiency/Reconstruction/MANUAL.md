...
On a new machine, configure dependencies once from the V4 root:
...
This automatically builds `../common/libeiid_common.a` if needed. The executable is `EIID_Recon_V4`.
...
| `absolute_efficiency_tree` | Tree containing `cell_index` and `sensitivity` |
...
| `absolute_efficiency_branches.efficiency` | Absolute-efficiency Branch, normally named `sensitivity` |
