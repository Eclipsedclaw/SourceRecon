...
该模块把紧凑 ROOT 事件、HEALPix 网格、绝对探测效率、EIID 响应函数和 LM-MLEM 求解器组装起来，可独立编译为 `EIID_Recon_V4`。
...
V4 在 LM-MLEM 分母中使用绝对效率。效率 `s_j <= 0` 的单元初值为零，迭代中始终保持为零，也不会被用作除数。

## 输入保证

若 Nside、排序、方向数、能量数或范围不匹配，或者缺少源半径、效率定义不属于 V4，`RootEfficiencyReader` 会直接报错。重复索引、缺失索引、越界索引和非法效率值由 `AbsoluteEfficiencyMap` 拒绝。
