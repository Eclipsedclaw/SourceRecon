...
V4 provides five user-facing programs:
...
| `Reconstruction/EIID_Recon_V4` | Run EIID + LM-MLEM reconstruction |
...
The project does not require a permanently activated Conda shell. Configure the machine once from the V4 root:
...
Subsequent terminals only need `make v4-all`; environment activation is unnecessary. After moving to another server, delete or overwrite `config/local.mk` by running `make configure` again.
...
| `absolute_efficiency_branches.efficiency` | Absolute-efficiency Branch, normally `sensitivity` |
