...
新机器先在 V4 根目录运行一次 `make configure`。如果脚本没有自动找到 Geant4，可明确指定：
...
把 `mode` 设为 `0`，程序会生成 `absolute_efficiency_master.root`。模板中的 `particles_per_cell = 100` 只适合冒烟测试；正式标定必须显著提高统计量，并分别增加粒子数和发射锥安全余量检查结果是否收敛。
