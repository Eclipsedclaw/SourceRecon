...
On a new machine, run `make configure` once from the V4 root. If Geant4 cannot be discovered automatically, use:
...
Use Mode 0 to create `absolute_efficiency_master.root`. Final production requires substantially more than the template's `100` particles per cell. Verify convergence by increasing the cone safety margin and particle count.
