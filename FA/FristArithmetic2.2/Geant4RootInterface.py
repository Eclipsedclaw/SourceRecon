...
class Geant4RootInterface:
    ...
    def get_raw_step_df(self):
        if self.raw_step_df is None:
            raise RuntimeError("请先调用 load()。")
        return self.raw_step_df.copy()

    def get_source_position_estimate(self):
        if self.source_position_estimate is None:
            return None
        return dict(self.source_position_estimate)
    ...
    def print_summary(self):
        ...
        if self.source_position_estimate is not None:
            estimate = self.source_position_estimate
            print("primary-gamma source position estimate / mm:")
            print(
                "    median = ({:.3f}, {:.3f}, {:.3f})".format(
                    estimate["x_median"],
                    estimate["y_median"],
                    estimate["z_median"],
                )
            ...
            print(
                "    robust spread = ({:.3f}, {:.3f}, {:.3f}), samples = {}".format(
                    estimate["x_robust_spread"],
                    estimate["y_robust_spread"],
                    estimate["z_robust_spread"],
                    estimate["n_samples"],
                )
            )
    ...
    def _build_hit_dataframe(self):
        ...
        px_branch = self._find_branch(["px_MeV", "px"], required=False)
        py_branch = self._find_branch(["py_MeV", "py"], required=False)
        pz_branch = self._find_branch(["pz_MeV", "pz"], required=False)
        creator_process_branch = self._find_branch(
            ["creatorProcess"], required=False
        )
        process_pre_branch = self._find_branch(["Process_pre"], required=False)
        process_post_branch = self._find_branch(["Process_post"], required=False)
        ...
        selected = {
            ...
            "py": py_branch,
            "pz": pz_branch,
            "creator_process": creator_process_branch,
            "process_pre": process_pre_branch,
            "process_post": process_post_branch,
        }
        ...
        optional_columns = [
            ...
            ("px_mev", px_branch, float),
            ("py_mev", py_branch, float),
            ("pz_mev", pz_branch, float),
        ]
        ...
        for column, branch in string_columns:
            if branch is None:
                df[column] = None
            else:
                df[column] = np.asarray(arrays[branch])
        ...
        self.raw_step_df = df.copy().reset_index(drop=True)
        self.source_position_estimate = self._estimate_source_position(df)
    ...
    def _estimate_source_position(self, df):
        """Estimate the Geant4 emission point from primary-gamma flight time.

        For a primary gamma with global time ``t`` at position ``r`` and
        incoming momentum direction ``u``, the emission point is approximately
        ``r - c*t*u``.  The median across events is robust against later
        scattered steps.  This is simulation-only geometry QA.
        """
        ...
        primary = primary.dropna(subset=numeric)
        primary = primary[primary["time_ns"] >= 0.0]
        if len(primary) == 0:
            return None

        # Use the earliest recorded primary-gamma step from each event, before
        # later interactions can change the momentum direction.
        primary = primary.sort_values(
            ["true_event_id", "time_ns"],
            kind="stable",
        )
        primary = primary.groupby("true_event_id", sort=False).head(1)

        momentum = primary[["px_mev", "py_mev", "pz_mev"]].to_numpy(float)
        ...
        directions = momentum / norms[valid, None]
        positions = primary[["x", "y", "z"]].to_numpy(float)[valid]
        times = primary["time_ns"].to_numpy(float)[valid]
        light_speed_mm_per_ns = 299.792458
        source_points = (
            positions
            - directions * (light_speed_mm_per_ns * times)[:, None]
        )
        finite = np.all(np.isfinite(source_points), axis=1)
        source_points = source_points[finite]
        if len(source_points) == 0:
            return None

        median = np.median(source_points, axis=0)
        q16 = np.quantile(source_points, 0.16, axis=0)
        q84 = np.quantile(source_points, 0.84, axis=0)
        spread = 0.5 * (q84 - q16)
        return {
            "x_median": float(median[0]),
            "y_median": float(median[1]),
            "z_median": float(median[2]),
            "x_robust_spread": float(spread[0]),
            "y_robust_spread": float(spread[1]),
            "z_robust_spread": float(spread[2]),
            "n_samples": int(len(source_points)),
        }
