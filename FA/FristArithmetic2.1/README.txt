...
程序会在训练前输出并检查：
    ...
    y_event_full 正负样本数；
    train/test same-gamma 正样本数；
    train/test full-absorption 正样本数。
...
matching_score 决定 hit 归属；imaging_weight 决定事件对图像的贡献。
...
目标：选择一组互不共享 hit 的 candidate，使 matching_score 总和最大。

    maximize sum(matching_score)
...
IMAGE_PLANE_Z=None 时，程序从第一散射层往源方向移动 100 mm 自动设置。
...
真值成像直接从完整 hit_df 按 true_event_id 分组，只接受：

    恰好 2 个 hit，且两个 hit 位于不同层；
    或恰好 3 个 hit，且三个 hit 分别位于不同层。

它不使用 fake window、模型分数、EventMatcher 或 train/test 切分。事件
...
模型选择结果保存在：
    ...
    output_images/geant4_training/model_imaging/
...
Geant4RootInterface.py
    读取 ROOT、映射 chamber/layer、过滤 step、聚合 hit、构造 event truth。

Geant4WindowBuilder.py
    按 true_event_id 切 train/test，并把多条 gamma 混成 fake windows。
...
Geant4TruthImager.py
    独立真值成像模块。直接按 true_event_id 构造跨层2-hit/3-hit事件，
    不使用 fake window、训练模型、matching 或 train/test 切分。
...
模型图片：
    ...
    程序目录/output_images/geant4_training/model_imaging/

实验图片：

    程序目录/output_images/experiment/
...
重点看 precision，而不是只看 loss。

第六步：检查 model_imaging 图
...
6. 当前 scorer 是线性模型。等几何、标签和概率校准稳定后，可升级为
...
6. train/test 必须按 true_event_id 切分，禁止按 candidate 随机切分。

7. truth_imaging 基准图不正确时，禁止解释 model_imaging 图。

8. loss 下降不等于物理正确，必须同时检查标签、precision、校准和图像。
