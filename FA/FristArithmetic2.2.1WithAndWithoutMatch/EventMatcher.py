...
class EventMatcher:
    ...
    def __init__(
        ...
        self.min_match_probability = float(min_match_probability)
    ...
    def _event_score(self, event):
        """
        Hit-ownership utility, separate from the imaging weight.

        A probability below the configured threshold has non-positive utility
        and is therefore not selected merely because sigmoid probabilities are
        always positive.  Multiplication by n_hits makes one 3-hit hypothesis
        comparable with hypotheses that cover the same number of detector hits.
        """
        match_prob = float(getattr(event, "match_prob", 0.0))
        if match_prob < self.min_match_probability:
            return 0.0

        if hasattr(event, "matching_utility"):
            return max(0.0, float(event.matching_utility))

        eps = 1e-9
        probability = min(max(match_prob, eps), 1.0 - eps)
        threshold = min(
            max(self.min_match_probability, eps),
            1.0 - eps,
        )
        ...
        return max(
            0.0,
            float(getattr(event, "n_hits", 1))
            * (log_odds - threshold_log_odds),
        )
