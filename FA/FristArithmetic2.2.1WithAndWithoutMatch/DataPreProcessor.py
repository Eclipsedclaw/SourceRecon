...
class DataPreProcessor:
    ...
    def build_final_df(self):
        ...
        # The current experimental interface is a one-hit-per-channel wide
        # table. Refuse to create a Cartesian product when an acquisition
        # window contains multiple hits in one channel. Such data must use the
        # future long-form coincidence-window interface instead.
        for channel_name, channel_df in (
            ("ch0", df0),
            ("ch1", df1),
            ("ch2", df2),
        ):
            duplicate_count = int(channel_df["EventID"].duplicated().sum())
            if duplicate_count > 0:
                raise ValueError(
                    channel_name
                    + " 中同一 EventID 存在多个 hit（重复行数="
                    + str(duplicate_count)
                    + "）。当前宽表流程会产生笛卡尔组合，请先构造长表时间窗。"
                )
