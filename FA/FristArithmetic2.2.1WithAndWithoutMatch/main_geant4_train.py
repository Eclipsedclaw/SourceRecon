...
def _generate_scored_model_image(
    event_list,
    image_plane_z,
    output_subdirectory,
    stage_name,
):
    """Save the same diagnostic set before or after EventMatcher."""
    ...
    print("sum of imaging weights:", float(np.sum(imager.event_weights)))
    print("figures:")
    for path in paths:
        print(path)
    print("==============================================")
...
def main(root_paths=None, dataset_configs=None):
    ...
    # Diagnostic A: use every scored candidate directly.  No hit-ownership
    # decision has been made, so this isolates scorer/imaging behaviour from
    # EventMatcher behaviour.
    _generate_scored_model_image(
        event_list=test_event_list,
        image_plane_z=image_plane_z,
        output_subdirectory="model_imaging_before_match",
    ...
    # Diagnostic B: use only the mutually non-overlapping events selected by
    # EventMatcher, with otherwise identical imaging parameters.
    _generate_scored_model_image(
        event_list=matched_test_event_list,
        image_plane_z=image_plane_z,
        output_subdirectory="model_imaging_after_match",
