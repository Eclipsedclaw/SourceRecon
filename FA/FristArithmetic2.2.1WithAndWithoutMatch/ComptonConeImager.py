...
class ComptonConeImager:
    ...
    def _get_event_compton_angle(self, event):
        """
        使用 candidate 的沉积能量和计算康普顿角。

        ``full_deposition_prob`` 已经连续地包含在成像权重中，因此这里
        不再使用 0.5 硬门限。否则概率校准后，大量低基准率但仍有信息的
        candidate 会突然全部消失，而不是按概率平滑降低贡献。
        """

        # Optional diagnostic interface. Production unknown-energy imaging
        # leaves this as None.  No source-energy truth enters this calculation.
    ...
    def plot_top_event_responses(
        ...
        # Keep only the requested top responses in memory.  Before matching,
        # an EventList can contain thousands of candidates; retaining one
        # n_pixels x n_pixels array per candidate can otherwise require
        # gigabytes of RAM.
        weighted_events = []
        sequence_number = 0
        ...
        for event in self.event_list.events:
            ...
            sequence_number += 1
            if len(weighted_events) < n_events:
                heapq.heappush(weighted_events, item)
            elif effective_peak > weighted_events[0][0]:
                heapq.heapreplace(weighted_events, item)
        ...
        for i, (_, _, weight, event, response) in enumerate(
            weighted_events
        ):
